...
const getResultsSurvey = async (req, res) => {
  ...
  try {
    const getParticipation = await client.query(
      "select S.Section_total_students, S.Section_no_of_votes, S.Position from Sections as S where S.Section_id = ($1)",
      [sectionId]
    );
    const {
    ...
    const getFreeText = client.query(
